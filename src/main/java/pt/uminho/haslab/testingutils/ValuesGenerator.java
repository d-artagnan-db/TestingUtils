...
public class ValuesGenerator {
	 ...
	 * reasons for this are that the sharemind dealer always uses a mod of nbits
	 * +1.If the generated value is 62 then the mod will be 63. Since the values
	 * are being exchanged by a protocol buffer message that is assuming longs,
	 * and the secrets are converted to longs, then the maximum value of a long
	 * is (2^63) -1. Long uses 64 bits, one for sign and the other 63 to hold
	 * the value.
	 */
	...
	public static Collection NRegionsRedisTestValueGenerator(int nvals) {
		Object[] parameters = new Object[1];

		Map<Integer, List<String>> ips = new HashMap<Integer, List<String>>();
		Map<Integer, List<Integer>> ports = new HashMap<Integer, List<Integer>>();
		Map<Integer, List<BigInteger>> requestIDs = new HashMap<Integer, List<BigInteger>>();
		Map<Integer, List<BigInteger>> regionIDs = new HashMap<Integer, List<BigInteger>>();

		String ip = "192.168.12.";

		ips.put(0, new ArrayList<String>());
		ips.put(1, new ArrayList<String>());
		ips.put(2, new ArrayList<String>());

		ports.put(0, new ArrayList<Integer>());
		ports.put(1, new ArrayList<Integer>());
		ports.put(2, new ArrayList<Integer>());

		requestIDs.put(0, new ArrayList<BigInteger>());
		requestIDs.put(1, new ArrayList<BigInteger>());
		requestIDs.put(2, new ArrayList<BigInteger>());

		regionIDs.put(0, new ArrayList<BigInteger>());
		regionIDs.put(1, new ArrayList<BigInteger>());
		regionIDs.put(2, new ArrayList<BigInteger>());
		...
		for (int i = 0; i < nvals; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				ips.get(j).add(destip);
				ports.get(j).add(port);
			}

			BigInteger requestID = new BigInteger(maxBits, generator);
			BigInteger regionID = new BigInteger(maxBits, generator);

			requestIDs.get(0).add(requestID);
			requestIDs.get(1).add(requestID);
			requestIDs.get(2).add(requestID);

			regionIDs.get(0).add(regionID);
			regionIDs.get(1).add(regionID);
			regionIDs.get(2).add(regionID);

		}

		Object[] parameter = new Object[4];
		parameter[0] = requestIDs;
		parameter[1] = regionIDs;
		parameter[2] = ips;
		parameter[3] = ports;
		parameters[0] = parameter;
	...
	public static Collection PeerConnectionManagerMultipleServerTestValueGenerator(
			int nservers, int nclients, int nMessages) {
		Object[] parameters = new Object[1];
		String ip = "localhost";
		int basePort = 6260;

		List<String> serverBindingAddress = new ArrayList<String>();
		List<Integer> serverBindingPort = new ArrayList<Integer>();

		List<String> clientBindingAddress = new ArrayList<String>();
		List<Integer> clientBindingPort = new ArrayList<Integer>();

		List<List<byte[]>> messagesToSend = new ArrayList<List<byte[]>>();

		for (int i = 0; i < nservers; i++) {
			serverBindingAddress.add(ip);
			serverBindingPort.add(basePort);
			basePort += 1;
		}

		for (int i = 0; i < nclients; i++) {

			int serverIndex = generator.nextInt(nservers);
			clientBindingAddress.add(serverBindingAddress.get(serverIndex));
			clientBindingPort.add(serverBindingPort.get(serverIndex));
			List<byte[]> clientMessages = new ArrayList<byte[]>();

			for (int j = 0; j < nMessages; j++) {
				clientMessages.add(new BigInteger(maxBits, generator)
						.toByteArray());
			}
			messagesToSend.add(clientMessages);
		}

		Object[] parameter = new Object[5];
		parameter[0] = serverBindingAddress;
		parameter[1] = serverBindingPort;
		parameter[2] = clientBindingAddress;
		parameter[3] = clientBindingPort;
		parameter[4] = messagesToSend;
		parameters[0] = parameter;
	...
	public static Collection AllToAllMessageExchangeDistributedCluster(
			int nRegions, int nMessages) {
		Object[] parameters = new Object[1];
		String ip = "localhost";
		int basePort = 9990;

		if (nRegions % 3 != 0) {
			throw new IllegalStateException("NRegions must be a divisor of 3");
		}

		List<String> bindingAddress = new ArrayList<String>();
		List<Integer> bindingPort = new ArrayList<Integer>();

		List<List<byte[]>> messagesToSend = new ArrayList<List<byte[]>>();

		for (int i = 0; i < nRegions; i++) {
			bindingAddress.add(ip);
			bindingPort.add(basePort);
			basePort += 1;
		}

		// A test with these function assumes that the RegionsServers all
		// exchange the same messages.
		List<byte[]> clientMessages = new ArrayList<byte[]>();

		for (int j = 0; j < nMessages; j++) {
			clientMessages
					.add(new BigInteger(maxBits, generator).toByteArray());
		}
		...
		List<List<BigInteger>> regionsRequestIdentifiers = new ArrayList<List<BigInteger>>();

		for (int j = 0; j < nRegions; j++) {
			regionsRequestIdentifiers.add(new ArrayList<BigInteger>());
		}
		...
		for (int i = 0; i < ngroups; i++) {
			...
			for (int j = 0; j < nRegions; j += 3) {
				BigInteger requestIdentifier = new BigInteger(maxBits,
						generator);
				int firstPeer = j + 1 % nRegions;
				int secondPeer = j + 2 % nRegions;
				regionsRequestIdentifiers.get(j).add(requestIdentifier);
				regionsRequestIdentifiers.get(firstPeer).add(requestIdentifier);
				regionsRequestIdentifiers.get(secondPeer)
						.add(requestIdentifier);
			}
		}

		Object[] parameter = new Object[4];
		parameter[0] = bindingAddress;
		parameter[1] = bindingPort;
		parameter[2] = messagesToSend;
		parameter[3] = regionsRequestIdentifiers;
		parameters[0] = parameter;
